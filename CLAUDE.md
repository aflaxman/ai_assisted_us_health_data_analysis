...
3. **Data lives in the project subdirectory** — `data/raw/` for source files, `data/derived/` for generated files
   - Raw source files (downloaded or moved in) go in `data/raw/`
   - Generated parquets/CSVs go in `data/derived/` (gitignored — regenerated by notebooks)
4. **Outputs in `outputs/`** — CSVs and markdown summaries written by analysis notebooks
5. **`requirements.txt`** in the project subdirectory; set up with `uv venv && uv pip install -r requirements.txt`
6. **Launch Jupyter from the project directory** — notebooks use `os.path.abspath('.')` for paths
7. **`.gitignore`** in the project subdirectory — ignore `data/derived/`, `data/nhis/`, `.ipynb_checkpoints/`
